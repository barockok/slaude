...
- [2026-09-09 — Harness abstraction: a vendor-neutral port (`HarnessSessionSpec`/`HarnessEvent`/`HookPoint`) so slaude can run on Codex/Pi/DeepSeek Harness; the `halt` vs `block` hook split is preserved in the vocabulary because collapsing it reintroduces the 2026-06-16 bug; tools are described as in-process `NeutralTool`s rather than MCP (Pi has no MCP yet hosts them natively); `negotiate()` degrades conveniences quietly and refuses to boot when the approval gate has no interactive equivalent](docs/site/_content/field-notes/2026-09-09-harness-abstraction.md)
