...
- [2026-08-02 — Audience share-tiers: per-page `audience:` frontmatter (`private|manager|team|public|user:<id>`) on agent-slice pages, enforced as a disclosure filter at every read path (gather per-hit, get_page keyed on source_id, list_pages/graph/think fail-closed strips); turn context → `AudienceGrant` in `resolveBrainScope`; allowed channels now read the agent slice filtered to `public` tier; self-declared tiers only RESTRICT own recall so destination-not-intent holds; sqlite `kb_page_audience` index written through on memoize](docs/findings/2026-08-02-audience-share-tiers.md)
