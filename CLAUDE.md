...
- [2026-09-17 — Gateway horizontal-scale requirements: a gateway refuses to boot on Socket Mode (single-leader websocket, replicas duplicate responses), on embedded storage for slaude data (checks the driver, since SLAUDE_DB=pg without a URL is PGLite and passed a dialect check), or with the brain on PGLite. The last was found by running the topology: the brain's home is on the shared volume and it deletes any lock at boot, so two gateway replicas cleared each other's live lock; fix is the brain's existing Postgres engine in its own slaude_brain database. Remote-mode brains are exempt; mono and node are exempt](docs/site/_content/field-notes/2026-09-17-gateway-scale-requirements.md)
- [2026-09-18 — MCP credential ownership: every MCP credential (the agent's shared identity per persona, and each person's) moves to one encrypted gateway store. Two in-pod measurements drove it: the agent's credential write renames over the path (a shared symlinked file is silently un-shared), and the agent rotates tokens itself. Whose credentials a turn gets is signed into the job token as runAs (the existing initiator claim is whoever sent the message, not whom the session runs as); nodes receive access tokens only, making the gateway the sole refresher by construction; refresh is single-flight across replicas because a rotating refresh token can be spent once. Also found: default-persona transcripts on nodes had never reached the shared volume, and mid-turn recovery depends on the provider publishing conformant RFC 8414 metadata](docs/site/_content/field-notes/2026-09-18-mcp-credential-ownership.md)
- [2026-09-17 — Portal identity binding: a Slack identity is joined to an account by *delivery* of an ephemeral onboarding link, not by a typed id or an email match — the signed token carries the team and Slack user so the redeemer cannot change whose identity it binds, single use falls out of the binding table's primary key rather than a redemption ledger, and redeeming is a POST behind a custom header because a GET could be triggered cross-site to bind an attacker's Slack user to a signed-in victim's account](docs/site/_content/field-notes/2026-09-17-portal-identity-binding.md)
- [2026-09-17 — Per-persona runtime bundles: the bundle was resolved per *tenant* (first persona by name order) and cached per tenant, so a second persona in a tenant never got its own credentials; the gateway-side resolution was the correctness bug while the node cache key is only an efficiency one (per-fetch ETag revalidation masks a colliding key — proven by a mutation test that passed until the assertion was rewritten to count revalidations). Plus two credential-home bugs from one dropped persona argument: false revocation in paste-back mode, dead tokens in loopback mode](docs/site/_content/field-notes/2026-09-17-control-plane-per-persona-runtime.md)
