...
- [2026-09-11 — /1on1 resume amnesia returns: `existsSync` follows symlinks, so a `projects/` link whose target moved read as "missing" and `symlinkSync` threw EEXIST into a swallowed catch — the dangling link survived every boot, locked threads had nowhere to write a transcript, and the resume miss self-heals silently. Fixed with an lstat-based link repair (also for `plugins/`), a non-destructive migration of legacy real `projects/` dirs, and a loud resume-miss log](docs/site/_content/field-notes/2026-09-11-1on1-resume-dangling-link.md)
