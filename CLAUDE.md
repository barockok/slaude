...
- [2026-08-10 — Webhook multi-replica design: session lease = thread ownership (claim = registration, `PUBLISH` count = liveness, TTL = crash net) + pub/sub event forwarding to the owner instance; sessions table stays owner-free; shared-transcript volume prerequisite; opt-in via `SLAUDE_CLUSTER` (Redis only imported when enabled — zero footprint for socket mode / single-replica webhook); platform-agnostic naming (`SLAUDE_INSTANCE_ID`, not pod/k8s-specific)](docs/findings/2026-08-10-webhook-session-lease.md)
