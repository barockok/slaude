...
- [2026-08-10 — Webhook multi-replica: session lease implemented (`src/cluster/`) — thread ownership (claim = registration, `PUBLISH` count = liveness, TTL = crash net) + pub/sub event forwarding to the owner instance; opt-in via `SLAUDE_CLUSTER` (Redis only imported when enabled). Session-table mirror for cross-instance cold-resume deferred — blocked on the shared-transcript volume prerequisite, not on difficulty; see doc's Follow-up section](docs/findings/2026-08-10-webhook-session-lease.md)
