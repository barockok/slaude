...
function extractDescription(indexPath: string): string {
  ...
  if (fm?.[1]) {
    try {
      ...
      if (typeof parsed.description === "string" && parsed.description.trim()) {
        return truncate(parsed.description.trim());
      }
    } catch {
...
export function loadKbs(): KbEntry[] {
  ...
  for (const entry of readdirSync(root)) {
    ...
    const description = extractDescription(join(dir, indexFile));
    out.push({ label: entry, description, path: dir, index_file: indexFile });
  }
  cached = out;
  return out;
}
