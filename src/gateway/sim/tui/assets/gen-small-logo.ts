// One-time generator: downscale logo.ansi (28×11 half-block cells = 28×22 px) to a
...
const src = readFileSync(join(dir, "logo.ansi"), "utf8").replace(/\n$/, "");
...
writeFileSync(join(dir, "logo-small.ansi"), result.join("\n") + "\n");
console.log(`wrote logo-small.ansi — ${TW}×${targetRows} cells (from ${W}×${lines.length})`);
