...
describe("StubAgent", () => {
  it("runs the 'reply' behavior, posting through the real Surface (parity path)", async () => {
    ...
    agent.attachGateway({ start: async () => {}, stop: async () => {}, __sessionCtx: () => ctx, __resolveMcp: () => undefined, __agentConnect: async () => "", __agentOneOnOne: async () => "", __agentMentionOnly: async () => "" });
  ...
  it("captures an unknown behavior as an error", async () => {
    const agent = new StubAgent();
    agent.attachGateway({ start: async () => {}, stop: async () => {}, __sessionCtx: () => undefined, __resolveMcp: () => undefined, __agentConnect: async () => "", __agentOneOnOne: async () => "", __agentMentionOnly: async () => "" });
    agent.setMcpResolver(() => ({}));
