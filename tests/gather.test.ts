...
describe("gather (unit, injected call)", () => {
  ...
  test("rankScore is 0 when a hit carries neither rerank_score nor score", () => {
    expect(rankScore({ slug: "p" })).toBe(0);
  });

  test("dedups slugless hits by content identity (hitKey JSON fallback)", async () => {
    // Hits with no slug and no chunk_id fall back to JSON identity: two
    // identical payloads collapse to one, two distinct ones both survive.
    const call = async () => [
      { source_id: "x", rerank_score: 0.5 },
      { source_id: "x", rerank_score: 0.5 },
      { source_id: "y", rerank_score: 0.4 },
    ];
    const hits = await gather("q", scope(["a"]), { call });
    expect(hits).toHaveLength(2);
    expect(rankScore(hits[0]!)).toBe(0.5);
  });
