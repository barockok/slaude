...
describe("loadKbs", () => {
  ...
  test("frontmatter description key used", () => {
    seedKb("test-kb", (dir) => {
      writeFileSync(join(dir, "README.md"), [
        "---",
        "description: Operator runbooks for acme services",
        "---",
        "# Runbooks",
        "first paragraph",
      ].join("\n"));
    ...
    const kbs = loadKbs();
    expect(kbs[0]!.description).toBe("Operator runbooks for acme services");
  });
  ...
  test("label matches directory name", () => {
    seedKb("acme-runbooks", (dir) => {
      writeFileSync(join(dir, "README.md"), "# Runbooks");
    });
    expect(loadKbs()[0]!.label).toBe("acme-runbooks");
  });
