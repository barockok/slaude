...
describe("detectMarketplaceSource", () => {
  ...
  test("non-github URL → generic git source with ref", () => {
    expect(detectMarketplaceSource("git@bitbucket.org:Amartha/openskill.git", "main"))
      .toEqual({ source: "git", url: "git@bitbucket.org:Amartha/openskill.git", ref: "main" });
  });
});
...
describe("deriveMarketplaceSlug", () => {
  ...
  test("ssh URL strips .git", () => {
    expect(deriveMarketplaceSlug("git@bitbucket.org:Amartha/openskill.git")).toBe("openskill");
  });
