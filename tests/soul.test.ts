  ...
  test("parses 'category: ids'", () => {
    writeFileSync(
      ...
      [
        "# Persona",
        "## Approvers",
        "- code: <@U0XXXXXXXXX> <@U06ENBS6PV1>  ; comment",
...
describe("loadApproverEntries + selectApprovers", () => {
  ...
  test("raw user IDs (no <@>) supported", () => {
    writeFileSync(
      paths.soul,
      ["# Persona", "## Approvers", "- U0XXXXXXXXX: anything"].join("\n"),
    );
  ...
  test("line without colon skipped", () => {
    writeFileSync(
      paths.soul,
      ["# Persona", "## Approvers", "- U0XXXXXXXXX", "- <@U002>: anything"].join("\n"),
    );
    const entries = loadApproverEntries();
