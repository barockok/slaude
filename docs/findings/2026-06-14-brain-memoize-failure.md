...
Three staging cases, **two distinct failure modes**, same user-visible symptom
...
- **Thread `C0XXXXXXXXX/1781143233` (2026-06-11) — write confirmed.** Operator:
...
- **Thread `C0XXXXXXXXX/1781168759` (2026-06-14) — retrieval missed it.**
...
Followed three more recall complaints to the live staging brain (PGLite snapshot +
...
- **Mode C (scope silo) is real:** a `/1on1` memoize lands in `user-<id>`, which
  ...
  OKR pages existed in BOTH `user-u0xxxxxxxxx` (from the `/1on1`) and `shared`
...
- `EMBEDDING_DIMENSIONS` 2560 → 1280 in deploy values.
...
3. **`EMBEDDING_DIMENSIONS` 2560 → 1280** — deploy one-liner; latent bug.
