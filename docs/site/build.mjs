...
function discover(group) {
  ...
  // Newest first. Date-stamped names sort correctly as plain strings; release
  // notes do not (v0.9.0 sorts above v0.30.0), so those compare part by part.
  const semver = (f) => f.replace(/^v/, '').replace(/\.md$/, '').split(/[.-]/).map((p) => (/^\d+$/.test(p) ? Number(p) : p));
  const bySemver = (a, b) => {
    const A = semver(a);
    const B = semver(b);
  ...
  return readdirSync(dir)
    ...
    .sort(group.sort === 'semver' ? bySemver : (a, b) => (a < b ? 1 : a > b ? -1 : 0))
    .map((f) => {
...
<script>(function(){try{var t=localStorage.getItem('slaude-theme');if(t)document.documentElement.dataset.theme=t;}catch(e){}})();</script>
</head>
...
<script>window.__SLAUDE_BASE__=${JSON.stringify(A)};</script>
