...
Required fields: `identity.name`, `manager.userId`, `mandate`. See [SOUL Guide](../reference/configuration.md) for the full schema.
...
|---|---|
| [Installation & Configuration](../reference/configuration.md) | Full env reference, `ANTHROPIC_BASE_URL` gateways, model selection, Docker vs Kubernetes |
| [Architecture](architecture.md) | Session lifecycle, gateway ↔ agent ↔ MCP flow, sqlite + PVC layout |
| [SOUL & Persona](../reference/configuration.md) | Persona schema, approvers, channel trust tiers, redaction |
