...
> **Prerequisites:** You have deployed slaude, the bot is in your workspace, and you have edited `SOUL.md`. If not, start with [Getting Started](../getting-started/index.md).
...
Without the `sessions.engaged` column, every disengage lasted zero messages: a disengaged thread still had a session row, so the next plain reply hit the "restore from row" path and re-engaged immediately. The column was added for [this finding](../../docs/findings/2026-06-11-engagement-disengage-durability.md). The suppressed-recording trick uses `continue:false` rather than `decision:"block"` — the latter discards the prompt before it is persisted, so the model would never see the gap ([finding](../../docs/findings/2026-06-16-reengage-hook-suppress.md)).
