...
2. If the current directory contains `SOUL.md`, the cwd becomes home (`cd my-agent && slaude` just works).
3. Otherwise `~/.slaude`.
...
Notes for each release live in `docs/releases/vX.Y.Z.md` from the first RC onward.
...
- [Configuration Reference](../getting-started/) — full env var and `SOUL.md` schema.
- [Architecture](../guides/) — agent loop, engagement gate, and approval flow internals.
- [Skills & Knowledge](../api/) — extending the agent with skills and KB wikis.
