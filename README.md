...
- **Manager session via DM** — the manager's DM thread doubles as a runtime-config console. From DM, the operator can ask the agent to install a new MCP server, add/remove tools, swap models, edit SOUL.md, or reconfigure approvers — instead of editing files on the host. The DM already runs as `trust="restricted"` (manager + backupManager only by the gate), so it's the natural privileged channel — no separate persona or thread required. Changes propagate to live sessions.
